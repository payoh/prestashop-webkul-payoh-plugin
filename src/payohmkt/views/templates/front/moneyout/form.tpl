{capture name=path}
	...
	<span class="navigation_page">{l s='Money out' mod='payohmkt'}</span>
{/capture}
{if $logged}
		...
		<div class="dashboard_content">
			<div class="dashboard">
				<div class="page-title">
					<span>{l s='Money out' mod='payohmkt'}</span>
				</div>
				<div class="wk_right_col">
				{if isset($success) && $success}
        			<p class="alert alert-success">
            			{l s='Your Money out has been successfully transfered.' mod='payohmkt'}
        			</p>
				{else}
					<h3>{l s='Wallet balance' mod='payohmkt'} <span class="label label-success">{displayWtPriceWithCurrency price=$bal currency=$currency}</span></h3>
					<form class="std" action="{$link->getModuleLink('payohmkt', 'moneyout',['add'=>1])|escape:'html':'UTF-8'}" method="post">
						<div class="form-group">
							<label for="id_iban" class="required control-label">{l s='Ibans' mod='payohmkt'}</label>
							<select class="form-control is_required validate" style="width: 205px" id="id_iban" name="moneyout_data[id_iban]">
								<option value="">{l s='Select a creditor account' mod='payohmkt'}</option>
						...
						<div class="form-group">
							<label for="amount_to_pay" class="required"  >{l s='Amount to transfert' mod='payohmkt'}</label>
						...
						<button type="submit" name="addMoneyoutSubmit" class="button btn btn-default button-medium">
							<span>{l s='Transfert' mod='payohmkt'}</span>
						</button>
